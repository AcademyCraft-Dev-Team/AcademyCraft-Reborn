...
public record ProgramEffect(int lifetimeTicks, AutoCloseable cleanup) {
    public ProgramEffect {
        if (lifetimeTicks < 0 || lifetimeTicks > 72000)
            throw new IllegalArgumentException("Effect lifetime outside 0..72000");
        Objects.requireNonNull(cleanup);
    }

    public static ProgramEffect completed() {
        return new ProgramEffect(0, () -> {
        });
    }
