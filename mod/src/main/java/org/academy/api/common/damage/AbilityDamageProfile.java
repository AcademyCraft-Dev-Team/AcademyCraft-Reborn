...
public record AbilityDamageProfile(ResourceKey<DamageType> damageType, DamageSettlement settlement,
    ...
    /**
     * Resolves a registered profile by key, failing loudly when it is absent.
     */
    public static AbilityDamageProfile require(ResourceKey<AbilityDamageProfile> key) {
        return Registries.DAMAGE_PROFILES.get(key)
                .orElseThrow(() -> new IllegalStateException("Missing damage profile " + key.identifier()))
                .value();
    }
    ...
    public static final class Builder {
        ...
        private Builder(ResourceKey<DamageType> type) {
            this.type = Objects.requireNonNull(type);
        }

        public Builder settlement(DamageSettlement value) {
            settlement = Objects.requireNonNull(value);
            return this;
        }

        public Builder bypassAbsorption(boolean value) {
            bypassAbsorption = value;
            return this;
        }

        public Builder compatibilityVersion(int value) {
            version = value;
            return this;
        }

        public AbilityDamageProfile build() {
            return new AbilityDamageProfile(type, settlement, bypassAbsorption, version);
        }
    }
}
