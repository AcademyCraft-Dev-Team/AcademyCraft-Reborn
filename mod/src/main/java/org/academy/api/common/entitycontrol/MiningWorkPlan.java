...
public final class MiningWorkPlan {
    ...
    public MiningWorkPlan(BlockWorkRegion region) {
        this.region = region;
    }
    ...
    public @Nullable BlockPos claim(UUID worker, Vec3 position, long now, Predicate<BlockPos> exposed) {
        ...
        targets.forEach((pos, target) -> {
            if (target.owner != null) faces.merge(face(pos), 1, Integer::sum);
        });
        BlockPos best = null;
    ...
    private static long face(BlockPos pos) {
        return ((long) (pos.getX() >> 2) << 32) ^ ((pos.getZ() >> 2) & 0xffffffffL);
    }
