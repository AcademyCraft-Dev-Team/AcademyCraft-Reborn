...
public final class WingFlightMotion {
    ...
    public static Vec3 step(Vec3 velocity, WingControlIntent input, int previousButtons,
        ...
        if (input.has(WingControlIntent.BACK))
            velocity = velocity.add(look.add(0, -0.35, 0).scale(-0.2 * movementScale));
        if (input.has(WingControlIntent.LEFT))
            velocity = velocity.add(new Vec3(look.z, -look.y + 0.15, -look.x).scale(0.2 * movementScale));
        if (input.has(WingControlIntent.RIGHT))
            velocity = velocity.add(new Vec3(-look.z, -look.y + 0.15, look.x).scale(0.2 * movementScale));
        return velocity;
    }
}
