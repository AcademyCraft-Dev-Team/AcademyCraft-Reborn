...
public record WorkSettings(Mode mode, boolean repeat, boolean harvest, boolean replant,
    ...
    public enum Mode {
        ...
        public boolean animals() {
            return this == SHEARING || this == MILKING || this == FEEDING;
        }
    }
    ...
    public boolean matches(LivingEntity entity) {
        if (filters.isEmpty()) return true;
        var matched = filters.stream().anyMatch(filter -> filter.startsWith("#")
                ? BuiltInRegistries.ENTITY_TYPE.wrapAsHolder(entity.getType()).is(TagKey.create(Registries.ENTITY_TYPE, Identifier.parse(filter.substring(1))))
