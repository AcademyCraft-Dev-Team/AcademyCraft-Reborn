...
import org.academy.api.common.damage.SkillDamageTypeResolver;
...
public final class AbilityDamageService {
    ...
    public interface Backend {
        boolean apply(ServerPlayer controller, LivingEntity target, Skill skill,
    ...
    public static void registerBackend(Backend backend) {
        AbilityDamageService.backend = Objects.requireNonNull(backend);
    }
    ...
    public record Request(float amount, float maximumHealthPart,
        ...
        public Request {
            ...
            if (damageType != null && damageProfile != null)
                throw new IllegalArgumentException("Choose type or profile");
        }

        public static Request of(float amount) {
            return new Request(amount, 0, null, null);
        }
    ...
    public static Result apply(ServerPlayer controller, LivingEntity target, Skill skill, Request request) {
        ...
        var type = request.damageType() != null ? request.damageType()
                : request.damageProfile() != null ? AbilityDamageProfile.require(request.damageProfile()).damageType()
        ...
        var applied = requireBackend().apply(
                controller, target, skill, type, request.amount(), request.maximumHealthPart());
    ...
    private static Backend requireBackend() {
        var current = backend;
        if (current == null) throw new IllegalStateException("Ability damage backend is not registered");
        return current;
    }
}
