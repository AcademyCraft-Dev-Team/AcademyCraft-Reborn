...
public final class GlowEffect {
    /**
     * 26.3 起 {@code OutputTarget} 已移除；渲染目标改由调用方在创建 RenderPass 时决定，
     * 这里暴露 glow 输入目标供 {@link Render#registerOutputTarget} 注册。
     */
    ...
    private RenderTarget ensureInput(RenderTarget mainRenderTarget) {
        var mainDepthFormat = mainRenderTarget.getDepthTexture() == null ? null : mainRenderTarget.getDepthTexture().getFormat();
        if (input != null
                && input.width == mainRenderTarget.width
                && input.height == mainRenderTarget.height
                && hasSameDepthFormat(input, mainDepthFormat)) {
            return input;
        }
        ...
        inputDescriptor = new RenderTargetDescriptor(
                ...
                new RenderTargetDescriptor.TextureProperties(new Vector4f(0), GpuFormat.RGBA8_UNORM),
                mainDepthFormat == null ? null : new RenderTargetDescriptor.TextureProperties(new Vector4f(0), mainDepthFormat)
        );
        input = Render.Buffers.getResourcePool().acquire(inputDescriptor);
        if (mainDepthFormat != null) {
            input.copyDepthFrom(mainRenderTarget);
        }
        return input;
    }

    private static boolean hasSameDepthFormat(RenderTarget target, GpuFormat depthFormat) {
        var depth = target.getDepthTexture();
        if (depth == null) {
            return depthFormat == null;
        }
        return depthFormat != null && depth.getFormat() == depthFormat;
    }
    ...
    public void process() {
        ...
        var descHalf = new RenderTargetDescriptor(
                width / 2, height / 2,
                new RenderTargetDescriptor.TextureProperties(new Vector4f(0), GpuFormat.RGBA8_UNORM), null
        );
