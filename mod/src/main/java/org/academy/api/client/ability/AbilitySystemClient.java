...
public final class AbilitySystemClient {
    ...
    public static AbilityCategory category = Registries.ABILITY_CATEGORIES
            .get(AcademyKeys.LEVEL0).orElseThrow().value();
    ...
    private static volatile @Nullable HudToggle hudToggle;

    /**
     * Implementation-provided hook for toggling the ability HUD visuals.
     */
    public interface HudToggle {
        void toggleActive();
    }

    /**
     * Called once by the HUD implementation; the hook owns the visuals.
     */
    public static void registerHudToggle(HudToggle toggle) {
        hudToggle = Objects.requireNonNull(toggle);
    }
    ...
    public static boolean canLevelUp() {
        ...
        if (getCategory() == Registries.ABILITY_CATEGORIES
                .get(AcademyKeys.LEVEL0).orElseThrow().value()) return level == 0;
    ...
    public static void setActiveHUD(boolean activeHUD) {
        ...
        var toggle = hudToggle;
        if (toggle != null) toggle.toggleActive();
    }
    ...
    public static int getEffectiveMaxStacks(Skill skill, int skillLevel) {
        ...
        var stackData = SKILL_DATA.get(
                AcademyKeys.skill("multiple_brain_domain_segmentation").identifier().toString()
        );
