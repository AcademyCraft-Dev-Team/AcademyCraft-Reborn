...
import static com.mojang.renderpearl.api.pipeline.RenderPipeline.builder;
...
public final class Render {
    ...
    private static final IdentityHashMap<RenderType, Supplier<RenderTarget>> OUTPUT_TARGETS = new IdentityHashMap<>();

    public static RenderType registerOutputTarget(RenderType type, Supplier<RenderTarget> target) {
        OUTPUT_TARGETS.put(type, target);
        return type;
    }

    public static RenderTarget getOutputTarget(RenderType type) {
        var target = OUTPUT_TARGETS.get(type);
        return target != null ? target.get() : Minecraft.getInstance().gameRenderer.mainRenderTarget();
    }
    ...
    public static void runBlitPass(
        ...
        try (
            ...
            for (var texture : textures) {
                renderPass.setUniform(texture.name(), texture.view(), texture.sampler());
            }
    ...
    public static final class RenderPipelines extends net.minecraft.client.renderer.RenderPipelines {
        ...
        public static final RenderPipeline MSDF_TEXT = builder()
                ...
                .withVertexBinding(1, VertexFormat.builder(1)
                        ...
                        .addAttribute("InstFade", GpuFormat.RG32_FLOAT)
                        .build()
                )
                .withPrimitiveTopology(PrimitiveTopology.QUADS)
                .build();

        public static final RenderPipeline BITMAP_TEXT = builder()
                .withLocation(academy("pipeline/bitmap_text"))
                .withVertexShader(R.shaders.core.msdf_text_instanced)
                .withFragmentShader(R.shaders.core.bitmap_text)
                .withBindGroupLayout(BindGroupLayouts.DYNAMIC_TRANSFORMS)
                .withBindGroupLayout(BindGroupLayouts.PROJECTION)
                .withBindGroupLayout(BindGroupLayouts.SAMPLER0)
                .withCull(true)
        ...
        public static final RenderPipeline GLOW_BLEND = builder(BLIT_SCREEN_SNIPPET)
                ...
                .withBindGroupLayout(
                        BindGroupLayout.builder()
                                .withUniform("BlurTexture1", UniformType.COMBINED_IMAGE_SAMPLER)
                                .withUniform("BlurTexture2", UniformType.COMBINED_IMAGE_SAMPLER)
                                .withUniform("BlurTexture3", UniformType.COMBINED_IMAGE_SAMPLER)
    ...
    public abstract static class RenderTypes extends net.minecraft.client.renderer.rendertype.RenderTypes {
        ...
        // TextureAtlas location constants are deprecated in this snapshot without a replacement.
        ...
        static {
            glowOutputTarget(IMAG_PHASE_PARTICLE_POST, ARC, POS_COLOR_QUADS_GLOW_ADDITIVE,
                    POS_COLOR_TRANGLES_GLOW_ADDITIVE, POS_COLOR_TRANGLES_GLOW, POS_COLOR_QUADS_GLOW_POST);
        }

        private static void glowOutputTarget(RenderType... types) {
            for (var type : types) {
                registerOutputTarget(type, () -> GlowEffect.glowTarget());
            }
        }
