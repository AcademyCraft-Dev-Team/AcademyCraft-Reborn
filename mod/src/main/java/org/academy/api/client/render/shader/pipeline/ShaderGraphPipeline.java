 ...
 * {@link GpuDevice#compilePipeline(RenderPipeline, ShaderSource, java.util.concurrent.Executor)} 写入管线缓存。</p>
 */
public final class ShaderGraphPipeline {
    ...
    public CompiledRenderPipeline precompile(GpuDevice device, ShaderGraphResult result) {
        return device.compilePipeline(result.pipeline(), shaderSource, Util.backgroundExecutor()).join().finishCompile();
    }
    ...
    private static RenderPipeline buildPipeline(Identifier fragment, Identifier vertex, int samplerCount) {
        ...
        if (samplerCount > 0) {
            ...
            for (var i = 0; i < samplerCount; i++) {
                samplerLayout.withUniform(SamplerBinding.uniformName(i), UniformType.COMBINED_IMAGE_SAMPLER);
            }
