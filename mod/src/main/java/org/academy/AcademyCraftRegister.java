...
import org.academy.api.common.arc.PathModifierTypes;
import org.academy.api.common.arc.PathTypes;
...
import org.academy.internal.common.ability.mentalout.MentaloutControlRosterBackend;
...
import org.academy.internal.common.ability.program.AbilityProgramServiceBackend;
...
import org.academy.internal.common.world.damagesource.AbilityDamageServiceBackend;
...
public final class AcademyCraftRegister {
    ...
    public static void register(IEventBus modEventBus) {
        ...
        org.academy.api.common.network.PacketTypes.PACKET_TYPES.register(modEventBus);
    ...
    private static void onCommonSetup(FMLCommonSetupEvent event) {
        event.enqueueWork(() -> {
            MentalControlApi.registerBackend(new MentalControlRuntimeBackend());
            MentalPerceptionApi.registerBackend(new MentalPerceptionRuntimeBackend());
            GroupControlApi.registerBackend(new GroupControlRuntimeBackend());
            MentalControlRosterApi.registerBackend(new MentaloutControlRosterBackend());
            AbilityDamageService.registerBackend(new AbilityDamageServiceBackend());
            AbilityProgramService.registerBackend(new AbilityProgramServiceBackend());
            SkillTuning.registerProvider(new SkillTuningCore());
            Skill.registerCastCostModifier((player, skill, baseCost, cost, discrete) ->
                    DarkmatterSixWings.Server.adjustCategoryCost(player, skill, baseCost, cost));
            Skill.registerCastCostModifier((player, skill, baseCost, cost, discrete) ->
                    discrete ? CurrentSymbiosis.Server.adjustNextCastCost(player, skill, cost) : cost);
