...
import org.academy.internal.server.entity.HealthRecovery;
    ...
    private enum Scenario {
        ...
        NATIVE_TOOL_PHASE_AND_INTEGRITY_ARE_OPERATIONAL(
            ...
            void run(GameTestHelper helper) {
                ...
                var tool = player.getMainHandItem();
                // 26.3: axe/shovel/hoe actions are expressed by the BLOCK_TRANSFORMER component.
                helper.assertTrue(tool.has(DataComponents.BLOCK_TRANSFORMER),
                        "Native tool does not expose the combined block transformer");
        ...
        CUT_PHASES_AND_SHARED_LAW_MARK_ARE_OPERATIONAL(
            ...
            void run(GameTestHelper helper) {
                ...
                betaTarget.setInvulnerableTime(0);
                helper.assertTrue(DarkmatterDisassemble.Server.tryAutomatedAttack(
                                player, betaTarget),
        ...
        REPAIR_TICK_HEALING_ACCEPTS_TRUE_DAMAGE(
            ...
            void run(GameTestHelper helper) {
                ...
                var dm = new LawDetonationDamageSource(
                        SkillDamageSource.from(
                                new DamageSource(types.getOrThrow(DamageTypes.DM_DAMAGE)),
                                Skills.DARKMATTER_CUT.get()));
                try {
                    helper.assertTrue(SkillDamageUtil
                    ...
                    var projection = ((HealthOffsetAccess) cow)
                    ...
                    assertClose(helper, 9, HealthRecovery.restore(cow, 9),
                    ...
                    TrueHealthOffsetRuntime.clear(cow);
                    cow.setHealth(100);
                    EntityControlApi.capTrueHealthTemporarily(cow, 50, 2);
                    assertClose(helper, 9, HealthRecovery.restore(cow, 9),
                    ...
                    var state = ((HealthOffsetAccess) player)
                            .academy$getHealthOffset();
                    ...
                    for (var tick = 1; tick <= 2; tick++) {
                        ...
                        TrueHealthOffsetRuntime.afterEntityTick(
                                new EntityTickEvent.Post(player));
                        assertClose(helper, 2 + 8.2f * tick, player.getHealth(),
                    ...
                    EntityControlApi.banHeal(player, 19.6f);
                    assertClose(helper, 0, HealthRecovery.restore(player, 10),
                    ...
                    EntityControlApi.allowHeal(player);
                    ...
                    assertClose(helper, 0.4f, HealthRecovery.restore(player, 1000),
