...
public final class MeltdownerProgramExecutionBridge {
    ...
    private static @Nullable ProgramDirection optionalDirection(ProgramInputView inputs, String port) {
        return inputs.first(port)
                .map(value -> (ProgramDirection) value.value())
                .orElse(null);
    }

    private static @Nullable ProgramWorldPosition optionalWorldPosition(
            ProgramInputView inputs,
            String port
    ) {
        return inputs.first(port)
    ...
    private static @Nullable ProgramBlockPosition optionalBlockPosition(
            ProgramInputView inputs,
            String port
    ) {
        return inputs.first(port)
