...
public final class DarkmatterLawMark {
    ...
    public static boolean damageDetonation(LivingEntity target,
                                           ...
                                           float amount) {
        if (!DarkmatterTargeting.isDarkmatterDamage(source) || DarkmatterTargeting.isNetworkMember(target))
            return false;
        if (source.getEntity() instanceof ServerPlayer owner && !DarkmatterTargeting.isAttackableBy(owner, target))
            return false;
