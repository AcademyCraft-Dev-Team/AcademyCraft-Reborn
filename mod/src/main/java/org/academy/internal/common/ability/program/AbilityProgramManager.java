...
import org.academy.api.common.ability.data.CommonSkillData;
...
public final class AbilityProgramManager {
    ...
    static boolean validBook(Identifier category, @Nullable ProgramBook book) {
        ...
        return book.slots().stream().allMatch(slot -> matchesCategory(slot, category));
    }

    private static boolean matchesCategory(ProgramBook.Slot slot, Identifier category) {
        if (slot.empty()) return true;
        var program = Objects.requireNonNull(slot.program());
        return program.schemaVersion() == AbilityProgram.CURRENT_SCHEMA_VERSION
                && program.category().equals(category);
    }
    ...
    static ProgramBook decodeStoredBook(Identifier category, @Nullable String encoded) {
        ...
        try {
            ...
            if (decoded.valid()) {
                var book = Objects.requireNonNull(decoded.book());
                if (book.schemaVersion() == ProgramBook.CURRENT_SCHEMA_VERSION
                        && book.slots().size() > 0
                        && book.slots().size() <= SLOT_COUNT
                        && book.slots().stream().allMatch(slot -> matchesCategory(slot, category))) {
    ...
    public static void executeTriggered(
            ...
            CommonProgramNodeCatalog.@Nullable MovementCondition movement
    ) {
        ...
        for (var slot = 0; slot < SLOT_COUNT; slot++) {
            ...
            var outcome = OutputControl.callWithoutOutputAdjustment(() -> adapter.execute(
                    Objects.requireNonNull(compiled.program()),
                    player,
    ...
    private static void requireAddonThread(ServerPlayer player, int slot) {
        ...
        if (!player.level().getServer().isSameThread())
            throw new IllegalStateException("Program API requires server thread");
    ...
    public static AbilityProgramService.Result saveForAddon(
        ...
        if (!addonAllowed(player, program))
            return addonRejected("Category is unavailable or precision operations are locked");
    ...
    public static ProgramBook bookForAddon(ServerPlayer player) {
        ...
        if (!isSupportedCategory(category) || !unlocked(player))
            throw new IllegalStateException("Precision operations are unavailable");
    ...
    public static AbilityProgramService.Result executeForAddon(
        ...
        if (!compiled.valid()) return new AbilityProgramService.Result(
                AbilityProgramService.Status.REJECTED, -1, "Invalid program", compiled.diagnostics());
        if (!ProgramTriggers.acceptsManualExecution(Objects.requireNonNull(compiled.program())))
            return addonRejected("Entry requires an automatic trigger");
        ...
        var adapter = Objects.requireNonNull(executionAdapter(program.category()));
        var result = OutputControl.callWithoutOutputAdjustment(() -> adapter.execute(Objects.requireNonNull(compiled.program()), player, 1, slot,
    ...
    private static @Nullable CategoryExecutionAdapter executionAdapter(Identifier category) {
    ...
    private static ProgramVmDiagnostic transactionDiagnostic(
            ProgramActionTransaction.@Nullable Result transaction,
    ...
    public static final class Server {
        ...
        public static void execute(ExecutePacket packet) {
            ...
            if (!ProgramTriggers.acceptsManualExecution(Objects.requireNonNull(compiled.program()))) return;
            if (ServerProgramScheduler.contains(
                    player.level().getServer(),
                    new ServerProgramScheduler.SessionKey(
                            player.getUUID(), category, program.id(), packet.slot)
