...
public final class DamagePolicyGameTests {
    ...
    private static void registerTests(RegisterGameTestsEvent event) {
        ...
        event.registerTest(AcademyCraft.academy("damage_policy"), new DamageTest(GameTestData.of(
    ...
    public static void pre(LivingDamageEvent.Pre event) {
        if (event.getEntity().getUUID().equals(targetId))
            event.setNewDamage(weaken ? 0.0f : event.getNewDamage() * 2.0f);
    }
    ...
    private static void verifyCategoryEffects(GameTestHelper helper, ServerPlayer player) {
        ...
        for (var hit = 0; hit < 4; hit++) {
            electric.setInvulnerableTime(0);
            electric.hurtServer(level, arc, 1);
        }
        helper.assertTrue(AbilityCategories.ELECTROMASTER.get().electricalCharge(electric) == 4,
                "Four ordinary arc hits must accumulate four points");
        var originalSpeed = electric.getAttributeValue(Attributes.MOVEMENT_SPEED);
        electric.setInvulnerableTime(0);
        electric.hurtServer(level, arc, 1);
        helper.assertTrue(AbilityCategories.ELECTROMASTER.get().isParalyzed(electric),
                "Fifth charge point must trigger paralysis");
        helper.assertTrue(AbilityCategories.ELECTROMASTER.get().electricalCharge(electric) == 0,
        ...
        var interruption = AbilityCategories.ELECTROMASTER.get().electricalInterruptionTicks(electric);
        ...
        helper.assertTrue(AbilityCategories.ELECTROMASTER.get().electricalCharge(blocked) == 0,
        ...
        AbilityCategories.AEROMANIP.get().damageEquipment(armored, 80, true);
        ...
        zombie.getRandom().setSeed(seed);
        Flashing.Events.onDrops(dropEvent);
