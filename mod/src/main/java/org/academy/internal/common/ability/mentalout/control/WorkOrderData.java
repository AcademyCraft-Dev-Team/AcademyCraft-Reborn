...
public final class WorkOrderData extends SavedData {
    public record Entry(String controller, String subject, Identifier source, Identifier dimension,
        ...
        public BlockWorkRegion region() {
            return new BlockWorkRegion(dimension, minimum, maximum);
        }

        public String key() {
            return controller + "/" + source + "/" + subject;
        }
    ...
    private WorkOrderData(List<Entry> entries) {
        entries.forEach(entry -> orders.put(entry.key(), entry));
    }
    ...
    public void remove(String key) {
        if (orders.remove(key) != null) setDirty();
    }
}
