...
public final class SkillVfxPacket extends Packet<ClientPacketListener, SkillVfxPacket> {
    ...
    private void write(ByteBuf b) {
        ...
        switch (state) {
            case SkillVfxState.Beam s -> {
                b.writeFloat(s.xRot());
                b.writeFloat(s.yRot());
                b.writeFloat(s.length());
                b.writeFloat(s.scale());
                b.writeFloat(s.sideOffset());
                ...
                b.writeByte(s.flags());
                if (s.reflected()) {
                    b.writeFloat(s.reflectionDistance());
                    b.writeFloat(s.returnLength());
            ...
            case SkillVfxState.Plasma s -> {
                ...
                if (s.launched()) {
                    Vec3.STREAM_CODEC.encode(b, s.target());
    ...
    private static SkillVfxPacket read(ByteBuf b) {
        ...
        SkillVfxState state = switch (type) {
            case 0 -> {
                ...
                if ((flags & 8) != 0) {
                    distance = range(b, 0, 4096);
                    returnLength = range(b, 0, 4096);
                    direction = vector(b);
                }
            ...
            case 1 -> {
                ...
                if (launched) {
                    ...
                    origin = vector(b);
                    progress = range(b, 0, 1);
                    chargeRate = range(b, 0, 1);
                }
            ...
            case 4 -> {
                ...
                if (frame > 3 || duration < 1 || duration > 80)
                    throw new IllegalArgumentException("Invalid smoke parameters");
    ...
    @Override
    public PacketType<ClientPacketListener, SkillVfxPacket> getPacketType() {
        return PacketTypes.SKILL_VFX.get();
    }
    ...
    public static final class Client {
        ...
        @SubscribePacket
        public static void handle(SkillVfxPacket packet) {
            SkillVfxClient.accept(packet);
        }
    }
}
