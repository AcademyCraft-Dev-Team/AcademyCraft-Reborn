...
public final class AreaTeleportState {
    ...
    public static @Nullable Region complete(UUID player, ResourceKey<Level> dimension, BlockPos second) {
        return complete(player, dimension, second, MAX_REGION_SIZE);
    }

    public static @Nullable Region complete(UUID player, ResourceKey<Level> dimension, BlockPos second, int maxAxis) {
        var state = STATES.get(player);
        if (state == null || state.dimension == null || state.pending == null
                || !state.dimension.equals(dimension)) return null;
    ...
    public static @Nullable Region selected(UUID player) {
        var state = STATES.get(player);
        return state == null ? null : state.selected;
    }

    public static @Nullable Region destination(UUID player) {
        var state = STATES.get(player);
    ...
    private static @Nullable Region destinationRegion(State state) {
    ...
    public record Snapshot(@Nullable ResourceKey<Level> dimension, @Nullable BlockPos pending,
                           @Nullable Region selected, @Nullable Region destination) {
    }
    ...
    private static final class State {
        ...
        private @Nullable Region selected;
