...
public final class ProgramNodeExtensionIndex {
    ...
    public static void freeze() {
        ...
        candidates.forEach((id, type) -> {
            ...
            for (var category : extension.scope().allowedCategories()) {
                if (AbilityProgramDefinitions.find(category) == null)
                    declarationFailures.add(id + " has no program category " + category);
            }
            for (var skill : extension.scope().requiredCapabilities()) {
                if (Registries.SKILLS.get(skill).isEmpty())
                    declarationFailures.add(id + " requires missing skill " + skill);
            }
        });
    ...
    private static <C> Registration validate(
        ...
        if (extension.schemaVersion() < 1)
            throw new IllegalArgumentException("Extension schema version must be positive");
