...
public final class PlayerControlSessionManager {
    ...
    public static boolean validateMovePlayer(ServerPlayer player, ServerboundMovePlayerPacket packet) {
        ...
        if (controlling != null && controlling.state != State.CLOSED) {
            correct(player, Objects.requireNonNull(controlling.controllerAnchor), packet.getYRot(player.getYRot()),
    ...
    private static PlayerControlFrame normalizeDirectFrame(ServerPlayer subject, PlayerControlFrame frame) {
        ...
        if (subject.isPassenger()) {
            var id = BuiltInRegistries.ENTITY_TYPE.getKey(Objects.requireNonNull(subject.getVehicle()).getType()).getPath();
    ...
    private static @Nullable LivingEntity raycastEntity(
    ...
    private static @Nullable Session pathSession(PathSessionToken token) {
    ...
    private record SerializedItemStack(@Nullable Tag tag) {
    ...
    private static final class Session {
        ...
        private final @Nullable ControlHandle handle;
        private final long readyDeadline;
        private final @Nullable Anchor controllerAnchor;
        ...
        private Session(
                ...
                ServerPlayer controller,
                ServerPlayer subject,
                @Nullable ControlHandle handle,
                long readyDeadline,
                long now,
                @Nullable Anchor controllerAnchor,
