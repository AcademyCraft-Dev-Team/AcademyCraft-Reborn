...
public class AbilityConfig {
    ...
    private @Nullable SkillSettings findSkill(String skillId) {
        if (skillId == null) return null;
    ...
    private static @Nullable String categoryFromRegistry(String skillPath) {
        try {
    ...
    private static boolean looksLikeSkillSettings(JsonObject object) {
    ...
    public static class Action implements TypeHandler<AbilityConfig> {
        ...
        @Override
        public TypeAdapter<AbilityConfig> getAdapter(Gson gson) {
            var delegate = gson.getAdapter(AbilityConfig.class);
            return new TypeAdapter<>() {
                @Override
                public void write(JsonWriter out, AbilityConfig value)
                        throws IOException {
                    if (value == null) {
                        out.nullValue();
                        return;
                ...
                @Override
                public AbilityConfig read(JsonReader in)
                        throws IOException {
        ...
        private static JsonObject toTrimmedTree(
                Gson gson, AbilityConfig config) {
        ...
        private static JsonObject skillToTrimmedTree(
                Gson gson, SkillSettings settings,
         ...
         */
        private static void regroupFlatSkills(JsonObject root) {
            ...
            for (var entry : flat.entrySet()) {
                ...
                var group = grouped.has(category)
                        ? grouped.getAsJsonObject(category) : new JsonObject();
        ...
        private static boolean hasFlatSkillEntries(JsonObject skills) {
