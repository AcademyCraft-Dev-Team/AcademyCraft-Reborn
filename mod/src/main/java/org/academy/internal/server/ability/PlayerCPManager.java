...
public class PlayerCPManager implements AbilitySubsystem {
    ...
    public boolean tryOccupation(UUID uuid, float amount, Skill skill, int iterationTicks,
                                 boolean isPermanent, @Nullable String stackGroup, int stackLimit) {
    ...
    public boolean replacePermanentOccupationsAndTryOccupation(
        ...
        if (plan.timedAmount() > 0) {
            var plannedSkill = Objects.requireNonNull(plan.timedSkill());
            occupations.add(new AbilityData.CpOccupationData(
                    plan.timedAmount(),
                    plan.iterationTicks(),
                    plannedSkill.getKeyString(),
                    false
            ));
        }
    ...
    private @Nullable MixedOccupationPlan createMixedOccupationPlan(
            UUID uuid,
            Skill skill,
            float permanentAmount,
            List<TimedOccupationCharge> timedCharges
    ...
    private @Nullable AtomicOccupationPlan createAtomicOccupationPlan(
            UUID uuid,
            Map<Skill, Float> permanentAmounts,
            @Nullable Skill timedSkill,
            float timedAmount,
    ...
    int getMaxStacks(UUID uuid, Skill skill, int skillLevel) {
        var base = SkillTuningCore.maxStacks(abilityConfig, skill, skill.getMaxStacks(skillLevel));
    ...
    public void onAdvancementEarn(AdvancementEvent.AdvancementEarnEvent event) {
        ...
        if (display != null && display.type() == AdvancementType.CHALLENGE) {
