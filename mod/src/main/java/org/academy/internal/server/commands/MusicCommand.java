...
public final class MusicCommand {
        ...
        CompletableFuture
                .supplyAsync(() -> {
                    try {
                        ...
                        if (credential == null) {
                            throw new IOException("no shared account for " + provider);
                        }
    ...
    private static int playlistSetMode(CommandContext<CommandSourceStack> context) {
        var raw = StringArgumentType.getString(context, "value").toUpperCase(Locale.ROOT);
        var store = PresetPlaylistStore.getInstance();
        try {
    ...
    private static void broadcastPlaylist(MinecraftServer server) {
        var store = PresetPlaylistStore.getInstance();
