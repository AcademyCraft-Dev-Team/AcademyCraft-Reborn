...
public final class SkillVfxMultiplayerGameTests {
    ...
    private static void registerTests(RegisterGameTestsEvent event) {
        for (int count : new int[]{2, 8, 16}) {
            ...
            event.registerTest(AcademyCraft.academy("skill_vfx_multiplayer_" + count), new Instance(GameTestData.of(
    ...
    private static final class Instance extends GameTestInstance {
        ...
        private Instance(TestData<Holder<TestEnvironmentDefinition<?>>> info, int count) {
            super(info);
            this.count = count;
        }
        ...
        public void run(GameTestHelper helper) {
            new Scenario(helper, count).start();
        }
        ...
        protected MutableComponent typeDescription() {
            return Component.literal("Multiplayer VFX dispatch");
        }
    }
    ...
    private static final class Scenario {
        ...
        void after(int ticks, Runnable action) {
            helper.runAfterDelay(ticks, () -> guarded(action));
        }
        ...
        void clearPackets() {
            observers.forEach(Observer::drain);
        }
        ...
        void start() {
            guarded(() -> {
                for (int i = 0; i < count; i++)
                    observers.add(new Observer(helper,
        ...
        void flightReentry() {
            ...
            after(2, () -> {
                ...
                after(1, () -> {
                    for (var observer : observers)
                        check(observer.drain().stream().filter(p -> p.state instanceof SkillVfxState.Burst b
        ...
        void finishLoad() {
            ...
            for (var observer : observers) {
                ...
                packetsTotal += packets.size();
                bytesTotal += observer.encodedBytes;
            }
