...
final class HotSpotClassPointerAccess {
    ...
    static long read(Object target) {
        ...
        return rawRead(Objects.requireNonNull(STATE.unsafe), target, STATE.capability.klassOffset,
                STATE.capability.wordBytes);
    }

    static long wordFor(@Nullable Class<?> type) {
        if (!STATE.capability.available || type == null) return 0L;
        return CLASS_WORDS.computeIfAbsent(type, key -> {
            try {
                return read(Objects.requireNonNull(STATE.unsafe).allocateInstance(key));
    ...
    static boolean writeAndVerify(Object target, long word) {
        ...
        var unsafe = Objects.requireNonNull(STATE.unsafe);
        try {
            rawWrite(unsafe, target, STATE.capability.klassOffset,
    ...
    private static boolean runDispatchProbe(State state) {
        ...
        var original = rawRead(unsafe, probe, state.capability.klassOffset,
                state.capability.wordBytes);
        long replacement;
        try {
            replacement = rawRead(unsafe, unsafe.allocateInstance(ProbeSafe.class),
        ...
        try {
            rawWrite(unsafe, probe, state.capability.klassOffset,
    ...
    private record State(@Nullable Unsafe unsafe, Capability capability) {
