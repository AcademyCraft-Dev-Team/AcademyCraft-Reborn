...
final class DispatchSubclassFactory {
    ...
    static void initializeForUse(GenerationResult result) throws Throwable {
        ...
        var support = Objects.requireNonNull(result.support());
        if (support.initialized) return;
        synchronized (support) {
            if (support.initialized) return;
            initializeGenerated(Objects.requireNonNull(result.dispatchType()), support.names, support.ledger);
    ...
    private static @Nullable String findFinalOverride(Class<?> originalType, Class<?> template,
                                                      Class<?> expectedBase) {
    ...
    private static GeneratedBytes generateBytes(Class<?> originalType, Class<?> template,
        ...
        source.accept(new ClassRemapper(generated,
                new SimpleRemapper(Opcodes.ASM9, Map.of(templateInternal, generatedInternal))));
    ...
    record GenerationResult(@Nullable Class<?> dispatchType, @Nullable String failureReason,
                            @Nullable GeneratedSupport support) {
