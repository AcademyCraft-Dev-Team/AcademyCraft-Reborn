...
public final class MentaloutRosterClientState {
    ...
    public static void setResyncRequester(@Nullable LongConsumer requester) {
    ...
    public static void applyFullStart(
            long revision,
            int totalChunks,
            int totalEntries,
            int stuporCp,
    ...
    public static boolean applyFullChunk(long revision, int chunkIndex, List<Entry> entries) {
        var transfer = pendingFull;
    ...
    public static void clear(long revision) {
        if (revision < snapshot.revision()) return;
        if (pendingFull != null && revision < pendingFull.revision) return;
        if (revision < requiredRevision) {
            requestResync(requiredRevision);
