...
public final class WideAreaInterferenceClientState {
    ...
    private static @Nullable Entity originalCamera;
    private static @Nullable CameraType originalCameraType;
    private static @Nullable UUID captureTargetUuid;
    ...
    public static void setSelectionPreview(@Nullable AABB box) {
        ...
        private void ensureSize(int width, int height) {
            if (target == null) {
                target = new TextureTarget(
                        "WideAreaInterference target feed", width, height, GpuFormat.RGBA8_UNORM, null);
