...
public final class KineticShockwaveRenderer
    ...
    private static void emitRing(PoseStack poseStack, float radius, float strength,
        ...
        poseStack.pushPose();
        if (yRot != 0.0f) poseStack.rotate(Axis.YP.rotationDegrees(yRot));
        if (zRot != 0.0f) poseStack.rotate(Axis.ZP.rotationDegrees(zRot));
        if (xRot != 0.0f) poseStack.rotate(Axis.XP.rotationDegrees(xRot));
