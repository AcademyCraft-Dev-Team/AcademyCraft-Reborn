...
public final class WingRenderer implements VfxRenderer<WingData> {
    ...
    public void render(VfxRenderContext ctx, List<? extends WingData> data) {
        ...
        var textureIds = new LinkedHashSet<Identifier>();
        for (var item : data) {
            if (item.kind() == WingKind.PLATINUM && item.layer() == WingData.Layer.STABLE) {
                textureIds.add(R.textures.white_wing);
                textureIds.add(R.textures.platinum_wing_starfield);
            } else {
                textureIds.add(item.layer() == WingData.Layer.ASCENSION
                        ? R.textures.white_wing_ascension
                        : tornadoTexture(item.kind()));
            }
        }
        var textures = new HashMap<Identifier, AbstractTexture>();
        for (var textureId : textureIds) {
            textures.put(textureId, textureManager.getTexture(textureId));
        }
        try (var renderPass = passEncoder.createRenderPass(
            ...
            for (var item : data) {
                ...
                if (item.kind() == WingKind.PLATINUM && item.layer() == WingData.Layer.STABLE) {
                    renderPass.setPipeline(RenderSystem.getCompiledPipeline(VfxPipelines.WING_RING_COSMIC));
                    ...
                    var mask = textures.get(R.textures.white_wing);
                    renderPass.setUniform("Sampler0", mask.getTextureView(), mask.getSampler());
                    var starfield = textures.get(R.textures.platinum_wing_starfield);
                    renderPass.setUniform("Sampler1", starfield.getTextureView(), starfield.getSampler());
                } else {
                    renderPass.setPipeline(RenderSystem.getCompiledPipeline(item.layer() == WingData.Layer.ASCENSION
