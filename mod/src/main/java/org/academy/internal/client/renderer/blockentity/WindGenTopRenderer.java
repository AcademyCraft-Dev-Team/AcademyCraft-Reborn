...
public final class WindGenTopRenderer implements BlockEntityRenderer<WindGenTopBlockEntity, WindGenTopRenderState> {
    ...
    public void submit(WindGenTopRenderState renderState, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
        ...
        nodeCollector.submitModel(MODEL, renderState, poseStack, RenderTypes.entityCutout(R.textures.model.wind_gen_top), packedLight, packedOverlay, 0);
        if (renderState.breakProgress != null) {
            nodeCollector.submitCrumblingOverlay(MODEL, renderState, poseStack, RenderTypes.entityCutout(R.textures.model.wind_gen_top), packedLight, packedOverlay, 0, renderState.breakProgress);
        }
        poseStack.pushPose();
        poseStack.translate(-0.5f, 1.5f, 0.5f);
