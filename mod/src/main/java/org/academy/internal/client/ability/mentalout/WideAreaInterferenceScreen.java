...
public final class WideAreaInterferenceScreen extends UiScreen {
    ...
    protected void onInit() {
        ...
        for (int index = 0; index < workCoordinates.length; index++) {
            ...
            field.setOnTextChanged(ignored -> {
                if (!fillingCoordinates) applyVertexFields();
            });
    ...
    private int calculateActionColumns(int controlsWidth) {
        ...
        if (available >= minimumWidth * OPERATION_LABELS.length + gap * (OPERATION_LABELS.length - 1))
            return OPERATION_LABELS.length;
    ...
    private List<WorkButton> workButtons() {
        ...
        if (workPage == 0) {
            ...
            if (editingVertices) {
                buttons.add(workButton(y, workText("back_to_range"), () -> {
                ...
                buttons.add(new WorkButton(new Rect(workPanel.x + inset, y, half, scaled(19)), workText("redraw"),
                        () -> {
                            armedAction = WideAreaInterference.Action.WORK;
                ...
                buttons.add(workButton(y + row, workText("input") + positionText(workInput), () -> {
                    pickingWorkPoint = "input";
                    armedAction = null;
                }));
                buttons.add(workButton(y + row * 2, workText("output") + positionText(workOutput), () -> {
                    pickingWorkPoint = "output";
                    armedAction = null;
                }));
                buttons.add(workButton(y + row * 3, workText("clear_points"), () -> {
                    workInput = null;
                    workOutput = null;
                }));
            }
    ...
    private void renderWorkPanel(ProgramUiGraphics graphics, int mouseX, int mouseY) {
        ...
        for (var entry : workButtons())
            button(graphics, entry.bounds, Component.literal(entry.label), mouseX, mouseY, true, false);
        if (workPage == 0) {
    ...
    private void loadWorkSettings() {
        WorkOrderClientState.settings().ifPresent(settings -> {
            ...
            workRepeat = settings.repeat();
            workHarvest = settings.harvest();
            workReplant = settings.replant();
            workDenyList = settings.denyList();
            workReach = settings.miningReach();
            workFilter.setText(String.join(", ", settings.filters()));
            workInput = settings.input().orElse(null);
            workOutput = settings.output().orElse(null);
    ...
    private void startWork() {
        ...
        if (workFirst == null || workLast == null || selectedTargets.isEmpty()) {
            workError = "need_range";
            return;
        }
        try {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (workOpen && WideAreaInterferenceClientState.isRtsView() && workPanel.contains(x, y)) {
            if (event.button() == InputConstants.MOUSE_BUTTON_LEFT) {
                ...
                if (workPage == 1 && editingVertices) {
                    for (int index = 0; index < workCoordinates.length; index++)
                        if (coordinateBounds(index).contains(x, y)) {
                            blurWorkInputs();
                            Arrays.fill(movementKeys, false);
                            Arrays.fill(zoomKeys, false);
                            return super.mouseClicked(event, doubleClick);
        ...
        } else if ((event.button() == InputConstants.MOUSE_BUTTON_RIGHT || event.button() == InputConstants.MOUSE_BUTTON_MIDDLE) && display.contains(x, y)
                && WideAreaInterferenceClientState.isRtsView()) {
            if (altDown()) WideAreaInterferenceClientState.clearHiddenBlocks();
            else {
                if (event.button() == InputConstants.MOUSE_BUTTON_RIGHT) anchorRtsCenter();
                WideAreaInterferenceClientState.stopPan();
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        if (dragMode == DragMode.PAN && (event.button() == InputConstants.MOUSE_BUTTON_RIGHT || event.button() == InputConstants.MOUSE_BUTTON_MIDDLE)) {
            if (event.button() == InputConstants.MOUSE_BUTTON_RIGHT)
                WideAreaInterferenceClientState.orbit(dragX, dragY);
            else WideAreaInterferenceClientState.panFromMouse(dragX, dragY);
            return true;
        }
        if (dragMode != DragMode.NONE && event.button() == InputConstants.MOUSE_BUTTON_LEFT) {
        ...
        if (WideAreaInterferenceClientState.isRtsView()) {
            switch (event.key()) {
                ...
                case InputConstants.KEY_Q -> {
                    anchorRtsCenter();
                    WideAreaInterferenceClientState.rotate(-5.0f);
                }
    ...
    private void anchorRtsCenter() {
        anchorRtsCenter(false);
    }
