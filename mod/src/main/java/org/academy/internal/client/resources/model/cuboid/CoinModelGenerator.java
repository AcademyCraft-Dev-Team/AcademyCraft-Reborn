package org.academy.internal.client.resources.model.cuboid;
    ...
    private static QuadCollection bake(TextureSlots textureSlots, ModelBaker modelBaker, ModelState modelState, ModelDebugName name) {
        ...
        if (frontMaterial != null && backMaterial != null) {
            singleResult = modelBaker.compute(modelBakery -> {
                ...
                var frontInfo = modelBakery.interner().materialInfo(BakedQuad.MaterialInfo.of(front, front.sprite().transparency(), 0, null, 0, true));
                var backInfo = modelBakery.interner().materialInfo(BakedQuad.MaterialInfo.of(back, back.sprite().transparency(), 0, null, 0, true));
