...
public final class TexturedQuadRenderer implements VfxRenderer<WorldQuadData> {
    ...
    public TexturedQuadRenderer(boolean glow) {
        this.glow = glow;
    }
    ...
    @Override
    public void render(VfxRenderContext context, List<? extends WorldQuadData> data) {
        ...
        var grouped = new LinkedHashMap<Identifier, List<WorldQuadData>>();
        for (var item : data) {
    ...
    private void renderBatch(
        ...
        for (var quad : data) {
            putCorner(instanceData, quad.corner0(), quad.uv0(), quad.color(), camera);
            putCorner(instanceData, quad.corner1(), quad.uv1(), quad.color(), camera);
            putCorner(instanceData, quad.corner2(), quad.uv2(), quad.color(), camera);
            putCorner(instanceData, quad.corner3(), quad.uv3(), quad.color(), camera);
        }
        instanceData.flip();
        ...
        try (var pass = encoder.createRenderPass(
            ...
            pass.setPipeline(RenderSystem.getCompiledPipeline(glow
    ...
    private String label() {
        return glow ? "VFX Textured Quad Glow" : "VFX Textured Quad";
    }
