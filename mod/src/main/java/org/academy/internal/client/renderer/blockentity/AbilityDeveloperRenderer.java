...
public final class AbilityDeveloperRenderer implements BlockEntityRenderer<AbilityDeveloperBlockEntity, AbilityDeveloperRenderState> {
    ...
    public void submit(AbilityDeveloperRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        if (renderState.isMain) {
            ...
            poseStack.rotate(Axis.YP.rotationDegrees(yRot + 180));
            poseStack.translate(0, 0, 1);

            submitNodeCollector.submitModel(MODEL, renderState, poseStack, Render.RenderTypes.ABILITY_DEVELOPER, renderState.lightCoords, OverlayTexture.NO_OVERLAY, 0);
            if (renderState.breakProgress != null) {
                submitNodeCollector.submitCrumblingOverlay(MODEL, renderState, poseStack, Render.RenderTypes.ABILITY_DEVELOPER, renderState.lightCoords, OverlayTexture.NO_OVERLAY, 0, renderState.breakProgress);
            }
