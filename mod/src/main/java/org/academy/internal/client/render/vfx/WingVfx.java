...
public final class WingVfx implements Vfx {
    ...
    public static BlackDiagnostics blackDiagnostics() {
        ...
        for (var playback : BLACK_ATTACKS.values()) {
            started += playback.timeline.started();
            duplicates += playback.timeline.duplicates();
            coalesced += playback.timeline.coalesced();
            expired += playback.timeline.expired();
        }
    ...
    private static void sampleBlackGraphs(VfxFrameContext ctx, Map<Integer, Matrix4f> capturedRoots) {
        ...
        for (var player : players) {
            ...
            graph.setLiveParam("sweep_left", Value.of(0f));
            graph.setLiveParam("sweep_right", Value.of(0f));
            graph.setLiveParam("pitch_left", Value.of(0f));
            graph.setLiveParam("pitch_right", Value.of(0f));
            graph.setLiveParam("attack_mode", Value.of(0f));
            graph.setLiveParam("attack_progress", Value.of(1f));
            ...
            for (int i = 0; i < attack.targets.size(); i++) {
                ...
                if (attack.pattern == VortexAttackPattern.FOURFOLD_SLAM)
                    graph.setLiveParam("attack_corner_" + i, Value.of(target));
                else {
    ...
    private static void renderSweepAnimations(PoseStack poseStack, VertexConsumer buffer,
        ...
        for (var entry : animations) {
            ...
            poseStack.rotate(new Quaternionf().rotateZ(-sweepYaw * Mth.DEG_TO_RAD));
            poseStack.rotate(new Quaternionf().rotateX(animation.pitchOffsetDeg * Mth.DEG_TO_RAD));
            poseStack.rotate(new Quaternionf()
                    .rotateZ((animation.leftWing ? 30.0f : -30.0f) * Mth.DEG_TO_RAD)
                    .rotateX(30.0f * Mth.DEG_TO_RAD));
