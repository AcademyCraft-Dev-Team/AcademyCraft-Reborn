...
public final class WorldLineOverlayPass {
    ...
    static {
        LevelRenderEvent.registerGeometryRouter(new LevelRenderEvent.GeometryRouter() {
            @Override
            public boolean accepts(RenderType renderType) {
                return WorldLineOverlayPass.accepts(renderType);
            }

            @Override
            public void submitCustomGeometry(LevelRenderEvent event, RenderType renderType,
                                             BiConsumer<MatrixStack, VertexConsumer> renderer) {
                WorldLineOverlayPass.submit(
                        event.poseStack(), renderType, event.getMatrixStack().copy(), renderer);
            }

            @Override
            public void submitPoseGeometry(LevelRenderEvent event, RenderType renderType,
                                           SubmitNodeCollector.CustomGeometryRenderer renderer) {
                WorldLineOverlayPass.submitPose(event.poseStack(), renderType, renderer);
            }
        });
    }
    ...
    public static void renderWorld(FeatureRenderDispatcher dispatcher, Matrix4fc modelViewMatrix) {
        ...
        try {
            ...
            withModelView(modelViewMatrix, () -> {
                ...
                var depthTexture = mainTarget.getDepthTextureView();
                if (colorTexture == null) return;
                try (var frame = dispatcher.prepareFrame(WORLD_STORAGE);
                     var renderPass = RenderSystem.getDevice()
                             .createCommandEncoder()
                             .createRenderPass(() -> "World line overlay", colorTexture,
                                     Optional.empty(), depthTexture, OptionalDouble.empty())) {
