...
class ImGuiBackend(
    ...
    fun init() {
        ...
        inputBackend.init(windowHandle)
        initialized = true
    }
    ...
    private fun renderDrawData(drawData: ImDrawData, renderPass: RenderPass, areaWidth: Int, areaHeight: Int) {
        ...
        for (n in 0 until drawData.cmdListsCount) {
            ...
            for (cmdI in 0 until drawData.getCmdListCmdBufferSize(n)) {
                ...
                renderPass.setUniform("Texture", binding.first, binding.second)
    ...
    fun onSdlEvent(event: org.lwjgl.sdl.SDL_Event) {
        inputBackend.processEvent(event)
    }
