...
class BackdropBlurEngine {
    ...
    fun fillRegion(
        ...
        encoder.createRenderPass({ "Backdrop fillRegion -> $target" }, target, Optional.empty()).use { rp ->
            rp.setPipeline(RenderSystem.getCompiledPipeline(Render.RenderPipelines.BACKDROP_SAMPLE))
            rp.setUniform("Sampler0", lo, sampler)
            rp.setUniform("Sampler1", hi, sampler)
    ...
    private fun ensureTargets(width: Int, height: Int, layerCount: Int) {
        ...
        for (i in 0 until layerCount) {
            val (lw, lh) = sizes[i]
            levels[i] = TextureTarget("BackdropBlur-L$i", lw, lh, GpuFormat.RGBA8_UNORM, null)
        }
        baseW = width
        baseH = height
