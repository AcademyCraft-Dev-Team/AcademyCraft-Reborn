...
open class UiContext {
    ...
    private val dynamicUniformStorages: MutableMap<Class<out DynamicGpuData>, DynamicGpuDataStorage<*>> =
        HashMap()
    ...
    private fun drawPrepared(
        ...
        val meshesToDraw = BatchProcessor.process(
            commands,
            object : UboUploader {
                override fun <T : DynamicGpuData> upload(payload: UniformPayload<T>): GpuBufferSlice {
    ...
    private fun <T : DynamicGpuData> uploadPayload(
        payload: UniformPayload<T>
    ): GpuBufferSlice {
        return getOrCreateUbo(payload.type, payload.size).writeData(payload.data)
    }

    @MainThread
    private fun <T : DynamicGpuData> getOrCreateUbo(
        uboClass: Class<T>, size: Int
    ): DynamicGpuDataStorage<T> {
        return UncheckedUtil.uncheckedCast<DynamicGpuDataStorage<T>>(
            dynamicUniformStorages.computeIfAbsent(
                uboClass
            ) { _ ->
                DynamicGpuDataStorageMapped<DynamicGpuData>(
                    uboClass.getSimpleName() + "_UBO", size,
                    GpuBuffer.USAGE_UNIFORM or GpuBuffer.USAGE_COPY_DST, 2
                )
            })
    }
