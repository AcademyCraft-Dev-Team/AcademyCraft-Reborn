...
layout(location = 10) in float flowCoord;
layout(location = 18) in vec2 materialUv;
layout(location = 25) in vec4 portalProjection;
...
float tailBand(float coordinate, float speed, float width) {
    float wrapped = fract(coordinate * 2.0 - Params.x * speed);
    float distanceToPeak = min(wrapped, 1.0 - wrapped);
    return 1.0 - smoothstep(0.0, width * 2.5, distanceToPeak);
}
...
void main() {
    ...
    // The CPU stream gives each segment a stable signed direction and speed.
    // Two additional seeded streams create irregular bright packets with
    // independent directions, speeds, and phase offsets.
    ...
    float tertiaryPulse = pulseBand(
            ...
            width * 0.55);
    float localNoise = 0.72 + 0.28 * sin(
            flowCoord * (18.0 + hash11(streamSeed + 127.0) * 11.0)
                    - Params.x * (0.02 + hash11(streamSeed + 139.0) * 0.04));
    float pulse = clamp(max(primaryPulse,
            max(secondaryPulse * 0.82, tertiaryPulse * 0.65)) * localNoise,
            0.0, 1.0);
    float tail = clamp(max(
            tailBand(flowCoord, speed, width),
            max(secondaryPulse * 0.55, tertiaryPulse * 0.40)),
            0.0, 1.0);
    ...
    float portalBrightness = max(portal.r, max(portal.g, portal.b));
    float intensity = band * clamp(
            0.16 + portalBrightness * 0.65 + tail * 1.20 + pulse * 3.20,
            0.0, 4.5);
    float alpha = vertexColor.a * band * clamp(0.12 + tail * 0.55 + pulse, 0.0, 1.0);
