...
class BoundedAnimationTimelineTest {
    @Test
    void delayedWorldEventsStartAtTheirFirstVisibleFrame() {
    ...
    @Test
    void jitteredFivePatternSequenceDoesNotRestartOrClipCurrentStroke() {
    ...
    @Test
    void duplicateAndBurstRecoveryAreBounded() {
        var timeline = new BoundedAnimationTimeline<String>(0);
        timeline.offer(1, "running", 0, .8, 0);
        timeline.sample(0);
        ...
        assertEquals(1, timeline.coalesced());
        assertEquals(1, timeline.duplicates());
        timeline.offer(4, "unseen", .9, .8, 0);
        assertNull(timeline.sample(3));
        assertFalse(timeline.hasEvents());
    }

    @Test
    void sustainedAttacksDoNotAccumulateFrameRoundingDelay() {
    ...
    @Test
    void longRenderStallDoesNotBackdateANewAttackToAnExpiredStroke() {
        var timeline = new BoundedAnimationTimeline<String>(0);
        timeline.offer(1, "old", 0, .8, 0);
        timeline.sample(0);
    ...
    @Test
    void trackingSnapshotStartsMidStrokeAndSequenceFloorRejectsHistory() {
