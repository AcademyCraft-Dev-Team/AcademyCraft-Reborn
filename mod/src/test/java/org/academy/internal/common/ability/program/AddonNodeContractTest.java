...
class AddonNodeContractTest {
    ...
    private record Node(String translation, ProgramValueType type, int revision,
                        boolean valid) implements ProgramNodeExtension<Integer> {
        @Override
        public Codec<Integer> configurationCodec() {
            return Codec.INT.fieldOf("value").codec();
        }
        ...
        @Override
        public ProgramNodeExecution<Integer> execution() {
            return (_, _, _) -> ProgramNodeStep.data(Map.of());
        }
    ...
    private static String fingerprint(Node node) {
        return ProgramNodeExtensionIndex.build(CATEGORY, Map.of(NODE, node)).fingerprint();
    }
