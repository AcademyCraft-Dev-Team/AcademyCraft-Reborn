...
public class AbilityDeveloperBlockItem extends BlockItem {
    public AbilityDeveloperBlockItem() {
        super(AcademyCraftBlocksFabric.ABILITY_DEVELOPER_BLOCK, new Properties());
    }
    ...
    protected boolean canPlace(BlockPlaceContext context, @NotNull BlockState state) {
        ...
        final boolean canPlace = AbilityDeveloperBlockFabric.getRotatedSubjectBlocks(pos, context.getHorizontalDirection()).stream().allMatch(blockPos -> level.getBlockState(blockPos).isAir() && level.isUnobstructed(state, blockPos, collisionContext));
