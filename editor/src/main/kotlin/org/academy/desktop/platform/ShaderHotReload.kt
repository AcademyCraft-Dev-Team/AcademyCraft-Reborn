 ...
 * [DesktopApplication.reloadShaderPipelines] 换上一份新的管线缓存 → 下一帧
...
class ShaderHotReload(private val root: Path) {
    ...
    fun scanNow() {
        ...
        if (changed) {
            try {
                DesktopApplication.reloadShaderPipelines()
                println("[shader-hot-reload] shaders changed, pipeline cache reloaded")
            } catch (_: Exception) {
            }
        }
    }
