...
import org.academy.desktop.platform.modResources
...
private class SkyStrikeCaptureApp(private val root: Path) : EditorApp {
    ...
    override fun renderBackground(target: RenderTarget) {
        if (capture == null) capture = TextureTarget("Sky strike capture", 768, 1024, GpuFormat.RGBA8_UNORM, GpuFormat.D32_FLOAT)
        val captureTarget = capture!!
        ...
        val path = modResources(root).resolve("assets").resolve("academy").resolve("vfxgraph")
            .resolve(shot.asset + ".json")
        ...
        val halfX = halfY * captureTarget.width / captureTarget.height
        ...
        val color = captureTarget.getColorTextureView() ?: return
        renderer!!.render(color, captureTarget.getDepthTextureView(), sim.buffer(), camera, true,
            specs, WorldTransform.identity(), false)
        glow!!.render(color, captureTarget.getDepthTextureView(), sim.buffer(), sim.arcBuffer(), camera,
            specs, captureTarget.width, captureTarget.height)
