...
private class RailgunCaptureApp(private val root: Path) : EditorApp {
    ...
    override fun renderBackground(target: RenderTarget) {
        ...
        if (captureTarget == null) captureTarget = TextureTarget("Railgun capture", 1280, 720, GpuFormat.RGBA8_UNORM, GpuFormat.D32_FLOAT)
        if (renderer == null) { renderer = VfxGraphRenderer(); glow = EditorGlow(renderer!!) }
        val outputTarget = captureTarget!!
        val shot = shots[index]
        val path = modResources(root).resolve("assets").resolve("academy").resolve("vfxgraph")
            .resolve("railgun_shot.json")
