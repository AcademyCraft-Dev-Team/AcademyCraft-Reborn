...
- 当前 92 个技能均能在技能设置界面发现相应的方案或策略入口。
...
- 原有技能按键执行该技能的当前方案。
- 增加一个全局“方案轮盘/切换”操作，不为 92 个技能分别注册四个槽位键。
...
- 为 92 个技能记录：输入阶段、服务端包、CP 路径、目标来源、持续状态和清理事件。
...
| 92 个适配器长期漂移 | 维护成本高 | 按执行原型复用 adapter 与契约测试 |
...
- 92 个技能均在第 11 节达到计划的 P/S/G/V/M 层级。
- 未保存任何方案的旧玩家体验保持不变。
