...
| 项目 | 1.21.1 | 26.2 | 差异 |
| --- | ---: | ---: | ---: |
| 已注册物品 ID | 38 | 27 | 仍缺失 14 个旧版 ID，共有 24 个 |
...
结论：最初确认的 21 个缺失旧版物品中已迁移 7 个，当前仍缺失 **14 个旧版物品注册项**。其中 **10 个独立物品 PNG 仍缺失**；风机显示屏已复用 `screen.png`，虚相位探矿器已恢复专用模型和渲染；另有 4 个方块物品不使用独立物品 PNG，但其方块模型和纹理整套仍缺失。
...
1. 当前剩余 14 个缺失物品在 26.2 中没有对应注册项或当前格式的物品定义；选择移植后需同步修改注册、创造模式页签、翻译和数据生成器。
