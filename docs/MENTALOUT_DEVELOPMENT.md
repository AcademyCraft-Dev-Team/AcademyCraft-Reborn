...
### 2026-09-22：心智破坏致死保护与精神防护剥离

- `运行时验收通过`：本次心智破坏的实际生命结算即将致死，且目标具有精神免疫、技能精神保护、标签抗性或已生效的玩家挣脱抗性时，保留 1 点生命；当前不足 1 点时保留原值，不产生治疗。普通无防护目标仍可被击杀。
- 临时保护通过公开的 `HealthLossGuards.protectDeath` 作用域执行，在伤害加成、减免及资源防御之后拦截致死生命写入。取消伤害、全部吸收、预览和被拒绝的生命写入均不触发剥离；作用域结束或异常退出后立即撤销保护。
- 成功保命后调用 `MentalControlApi.suppressMentalProtection`，立即结束当前施术者的这次心智破坏，并释放它持有的呆然和反应减速句柄。其他施术者的独立效果仍遵守原来的引用计数规则。
- 清除实例免疫来源、玩家挣脱进度、自动抗性累积与手动／自动抗性计时；通过统一门禁屏蔽标签、矢量防护、未元六翼及电磁场等提供的精神保护。抑制期间禁止重新注册免疫、积累自动抗性或建立输入挣脱挑战；不移除这些技能的其他防御效果。
- 抑制采用内存状态，沿用死亡、重生、实体离开维度、卸载、登出及停服清理规则，不跨重登持久化。公开恢复 API 允许重新获得防护，但不会恢复已清除的旧实例来源和旧挣脱计时。
- 同步更新中英文技能说明、剥离提示与技能使用指南。

验证记录：

- `test -DisDev=true`：2122 项通过；开发／发布 `build` 均通过，编辑器 103 项测试通过，构建包含专用 JVM 配置检查。
- 分别以 `-PacademyGameTests=academy:<名称>` 运行 `mental_defense`、`damage_penetration`、`electromaster_rework_health`，全部通过。当前原版筛选器只接受单个通配模式，不支持逗号组合。
- `mental_defense` 新增的实际脉冲回归覆盖：非致死／致死、低于 1 点生命、出力控制放大伤害、抵抗效果使伤害归零、资源完全吸收、标签抗性、玩家手动抗性、呆然与减速清理、被动重新注册、自动抗性累积、输入挑战和重生清理；同时验证临时保护嵌套、拒绝写入及异常释放。
- `runClientDev -DisDev=true -I tools/vfxgraph-editor/scripts/mind-destruction-client.init.gradle`：真实施法网络包、服务器周期脉冲、客户端生命同步、提前结束、减速恢复、被动门禁与离开世界清理全部通过。脚本仅用于既有的 `run/ability-vfx/railgun` 隔离验收世界。
- **待验证**：扩展回归 `time_player_tick_scaling` 两次停在 tick 17 的基础半速测试，期望实体 phase 增加 3，实际为 0，尚未进入该用例的心智破坏阶段。2026-09-13 的既有 `build/paralysis-immunity-time.log` 已记录同样失败，随后的旧重试日志通过；本轮不据此宣称该专项通过。本次心智破坏自身的减速释放已由上述 GameTest 与真实客户端另行验证。
