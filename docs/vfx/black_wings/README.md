...
每侧一条带局部开口的核心、18 条螺旋细流、24 条短墨屑和 8 条移动高光，共 102 条曲线。
高光贴合现有螺旋细流，以短脉冲形式由根部向外运动，端部淡入淡出，主体仍保持墨黑。
...
- `./gradlew test -DisDev=true`：通过，含持续播放、左右横扫隔离、收束和恢复，以及高光位移、镂空断面和桥接细流测试。
- `./gradlew build -DisDev=true`：通过，包含编辑器测试。
- `./gradlew build -DisDev=false`：通过。
- MCP 图结构校验：通过，0 错误、0 警告。
- 本版在编辑器加载并播放超过 80 秒，102 条曲线，无 previewError；目视确认亮紫短带移动及局部空隙。
