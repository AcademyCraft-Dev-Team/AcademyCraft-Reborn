...
[五招实际速度预览（7.5 秒，每招 1.5 秒）](attack_motion.mp4)

五个独立图可在编辑器中直接打开、循环播放。游戏与编辑器都用同一个公开
`VortexAttackGeometry` 采样器和 1.5 秒时间线。鞭打用逐段方向积分，弯曲从根部传至尖端，
翼尖约滞后 0.36 秒；落下后有少量继续甩动，再平滑恢复已确认的待机形状。

游戏中由 `BlackWing.Server` 在成功接受攻击后选择下一招，通过 `BlackWingAttackPacket`
广播模式、服务器起始 tick 和固定世界目标。轮换为双翼下抽 → 左翼侧抽 → 右翼侧抽 → 压缩突刺 → 四翼砸地 → 重复。
公开 `VortexAttackSequence` 独立控制 30 tick 动作间隔，连续点击不会截断当前动作、额外扣费或跳过招式。
...
- `attack_progress`：0–1 为动作进度；负数用于编辑器自动按 1.5 秒循环。
...
- `./gradlew test -DisDev=true`：1614 项通过，0 失败、0 错误。
...
- 本轮重启 VFX 编辑器加载新 Java 实现并检查五个攻击图。
- 游戏内的旧客户端需要重启才能载入新的 Java 动画。桌面控制工具仍受 `apply deny-read ACLs`
