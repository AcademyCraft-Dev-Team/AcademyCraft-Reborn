...
- 已注册的唯一节点类型：271 个。
- 编辑器当前可见的唯一节点类型：206 个；被合并节点仍注册为隐藏兼容节点。
...
- 非入口节点中可由配置调整的文本、标签和数值字段会同时暴露同名的可选输入端口；未连接时沿用检查器配置，连接后以运行时输入覆盖。改变端口结构或类型的配置（例如集合输入数量和各类模式枚举）以及布尔开关、维度等其他配置仍只在检查器中调整。标签值可直接连接文本配置端口，并以带 `#` 前缀的资源标签文本补充选择器。
...
| `value/text` | 数值 | `value` | `[value:TEXT] → value:TEXT` | 输出文本；可选端口未连接时使用检查器内容。 |
| `value/tag` | 数值 | `value` | `[value:TAG] → value:TAG` | 输出资源标签；连接到文本配置端口时转换为带 `#` 前缀的文本。 |
...
| `flow/wait` | 流程/挂起 | — | `flow:F,ticks:I → flow:F` | 挂起 1–72000 tick 后继续。VM 会话、变量、循环状态、触发快照和已结算行动的补偿句柄跨 tick 保留；不会阻塞服务器线程。 |
...
冲击波节点的默认配置为 `power=1`、`destroy_blocks=false`、`radius=8`。`radius` 同时用于伤害判定和方块破坏；`radius=0` 不扫描邻近方块，只处理 `BlockPos.containing(position)`。伤害仍会乘以服务端能力威力与伤害倍率，方块破坏还必须通过全局及技能级破坏权限。旧 `strength/damage/block_radius` 配置在编辑器打开时迁移为新字段。
...
| `query/owned_jet_nozzle_count` | 查询 | 高速射流 | `center:WP,radius:N,[direction:D] → count:I` | 统计中心附近属于施术者的外置方块喷口；连接方向时只计入与方向夹角不超过 45° 的喷口。 |
| `action/launch_block_structure` | 行动 | 高速射流 | `flow:F,block:BP,direction:D → flow:F` | 方块必须安装施术者的喷口；以该方块为外表面中心、沿推动方向向内一格为球心，选取 `radius=2..8` 的球形区域。最小范围完整包含向内的 3×3×3 区域，随后沿喷口反向递归裁剪紧邻受阻部分，再原子化捕获并按质量推动。输入方向用于选择反向推力与之相符的喷口；另可配置 `power=0.00..2.00`、`duration=1..60` 秒。`restore_when_settled` 仅保留配置兼容，结构静止后始终结算为方块或掉落物。 |
...
| 喷流发射结构 | 球形半径由配置决定，速度上限 3 | 球形半径由配置决定，速度上限 6 | 球形半径由配置决定，速度上限 8 |
...
喷流发射只使用外置方块喷口，喷口的支撑方块不会并入结构。单喷口容量固定为 4 方块，结构总上限为 256 方块；有效喷口必须由施术者拥有、处于配置半径内、与输入方向夹角不超过 45°，且实际射流走廊与结构外包围盒相交。推动力按结构质量换算加速度。结构对沿途实体使用扫掠碰撞检测；法向闭合速度低于 0.35 时不触发伤害或击退，同一实体的碰撞效果有 10 tick 冷却。
...
11. **行动按流程依次结算**：普通分类在 VM 到达行动节点时立即执行“校验→应用”，所以可以表达“先施放、再等待、再施放”；等待期间保留补偿句柄，后续节点失败、会话取消或过期时按逆序尽力回滚。心理掌握的持续控制还需要先完成整图 CP 占用预检，因此仍在最终提交阶段执行，但同样按节点顺序逐项“校验→应用”，不再先统一校验后批量应用。
