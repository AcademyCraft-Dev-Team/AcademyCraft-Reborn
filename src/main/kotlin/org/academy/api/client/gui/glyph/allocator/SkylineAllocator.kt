package org.academy.api.client.gui.glyph.allocator

/**
 * Skyline 二值 packing（对标 stb/skyline）：在固定尺寸图集页里分配矩形槽位。
 * 只分配不释放；LRU 驱逐若需要，由上层在页满时重建或另建页。
 */
class SkylineAllocator(private val width: Int, private val height: Int) {
    ...
    private fun addSkylineNode(index: Int, x: Int, y: Int, w: Int) {
        skyline.add(index, SkylineNode(x, y, w))
