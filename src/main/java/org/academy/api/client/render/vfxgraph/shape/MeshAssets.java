...
public final class MeshAssets {
    ...
    public static float[] sphere(float radius, int segments) {
        var seg = Math.max(4, segments);
        var rings = Math.max(3, seg / 2);
        var out = new ArrayList<Float>();

        float[] top = {0, radius, 0};
        float[] bottom = {0, -radius, 0};
        for (var s = 0; s < seg; s++) {
            var a0 = (float) (s * Math.PI * 2 / seg);
            var a1 = (float) ((s + 1) * Math.PI * 2 / seg);
            // 顶盖（r=0 极点 → r=1）
            var t0 = spherePoint(a0, 1, rings, radius);
            var t1 = spherePoint(a1, 1, rings, radius);
            appendTri(out, top, t1, t0);
            // 底盖（r=rings-1 → r=rings 极点）
            var b0 = spherePoint(a0, rings - 1, rings, radius);
            var b1 = spherePoint(a1, rings - 1, rings, radius);
            appendTri(out, bottom, b0, b1);
            // 中间环带（r=1..rings-1）
            for (var r = 1; r < rings - 1; r++) {
                var a = spherePoint(a0, r, rings, radius);
                var b = spherePoint(a1, r, rings, radius);
                var c = spherePoint(a0, r + 1, rings, radius);
                var d = spherePoint(a1, r + 1, rings, radius);
                appendTri(out, a, b, d);
                appendTri(out, a, d, c);
            }
        }
