...
public class OverlappingPSDFCombiner implements ContourCombiner<Double> {
    private final int[] windings;
    private final PerpendicularDistanceSelector[] edgeSelectors;
    private Point2 p = new Point2();

    public OverlappingPSDFCombiner(Shape shape) {
        var count = shape.contours.size();
        windings = new int[count];
        edgeSelectors = new PerpendicularDistanceSelector[count];
        for (var i = 0; i < count; ++i) {
            windings[i] = shape.contours.get(i).winding();
            edgeSelectors[i] = new PerpendicularDistanceSelector();
        }
    }
