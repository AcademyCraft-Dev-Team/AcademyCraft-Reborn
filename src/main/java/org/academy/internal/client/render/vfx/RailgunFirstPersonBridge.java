...
public final class RailgunFirstPersonBridge implements EffectRenderer {
    public static final RailgunFirstPersonBridge INSTANCE = new RailgunFirstPersonBridge();
    private static final int RING_SEGMENTS = 28;

    private RailgunFirstPersonBridge() {
    }
    ...
    public void render(PoseStack poseStack, SubmitNodeCollector collector, int packedLight,
        ...
        // Third-person railgun effects are rendered by RailgunVfx through the Vfx pipeline.
    }
    ...
    public void renderFirstPerson(PoseStack poseStack, SubmitNodeCollector collector,
        ...
        submitHandRings(poseStack, collector, player.tickCount + partialTick, strength);
        if (!data.released()) {
        ...
        if (data.coinReturnHint()) {
            ...
            submitCoinReturnHint(poseStack, collector, player.tickCount + partialTick);
            poseStack.popPose();
        }
    }
}
