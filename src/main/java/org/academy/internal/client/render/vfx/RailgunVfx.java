...
public final class RailgunVfx implements Vfx {
    ...
    private static final Identifier COIN_TEXTURE = academy("textures/item/coin.png");
    ...
    public void sample(VfxFrameContext ctx, VfxSink sink) {
        ...
        for (Map.Entry<Integer, Matrix4f> entry : roots.entrySet()) {
            ...
            if (!data.released()) {
                submitCoin(sink, root, local, age, 0.23f);
            }
    ...
    private static void submitCoin(VfxSink sink, Matrix4f root, Matrix4f local,
                                   float time, float scale) {
        var world = new Matrix4f(root).mul(local)
                .rotateX(time * 50.0f * Mth.DEG_TO_RAD)
                .scale(scale, scale, scale);
        var half = 0.5f;
        var p0 = new Vector3f(-half, -half, 0.0f).mulPosition(world);
        var p1 = new Vector3f(half, -half, 0.0f).mulPosition(world);
        var p2 = new Vector3f(half, half, 0.0f).mulPosition(world);
        var p3 = new Vector3f(-half, half, 0.0f).mulPosition(world);
        sink.push(new RailgunRingData(
                COIN_TEXTURE,
                p0, p1, p2, p3,
                UV0, UV1, UV2, UV3,
