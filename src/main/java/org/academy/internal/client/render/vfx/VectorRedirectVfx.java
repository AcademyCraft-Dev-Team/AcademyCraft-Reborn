...
public final class VectorRedirectVfx implements Vfx {
    ...
    public VectorRedirectVfx(
        ...
        this.radius = Float.isFinite(radius) ? Mth.clamp(radius, 0.02f, 4.0f) : 0.25f;
        this.kind = kind;
        this.style = style;
    ...
    public void sample(VfxFrameContext context, VfxSink sink) {
        var age = Mth.clamp((System.nanoTime() - createdAt) / (double) LIFETIME_NANOS, 0.0, 1.0);
        ...
        var yRot = (float) (Mth.atan2(-direction.x, direction.z)) * Mth.RAD_TO_DEG;
        var xRot = (float) (Mth.atan2(-direction.y, horizontalLength)) * Mth.RAD_TO_DEG;
