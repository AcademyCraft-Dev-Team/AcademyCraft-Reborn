    ...
    public static void apply(ClassNode owner) {
        ...
        for (var caller : owner.methods) {
            ...
            for (var instruction : caller.instructions.toArray()) {
                ...
                for (var node : body.instructions) if (node instanceof LabelNode label) labels.put(label, new LabelNode());
                ...
                for (var node : caller.instructions.toArray()) if (node instanceof FrameNode) caller.instructions.remove(node);
                replaced++;
            }
        }
