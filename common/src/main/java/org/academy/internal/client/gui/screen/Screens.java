...
public class Screens {
    public static final Map<String, BiConsumer<ClientPacketListener, FriendlyByteBuf>> SCREEN_IDS = new HashMap<>();
    public static final BiConsumer<ClientPacketListener, FriendlyByteBuf> WIND_GEN_SCREEN =
            registerScreen(WindGenBaseBlock.WIND_GEN_SCREEN,
                    (listener, buf) -> {
    ...
    public static final BiConsumer<ClientPacketListener, FriendlyByteBuf> ABILITY_DEVELOPER_SCREEN =
            registerScreen(AbilityDeveloperBlock.ABILITY_DEVELOPER_SCREEN,
                    (listener, buf) -> {
                        BlockPos pos = buf.readBlockPos();
                        Minecraft.getInstance().setScreen(new AbilityDeveloperScreen(pos));
                    });
    public static final BiConsumer<ClientPacketListener, FriendlyByteBuf> WIRELESS_NODE_SCREEN =
            registerScreen(WirelessNodeBlock.WIRELESS_NODE_SCREEN,
    ...
    public static BiConsumer<ClientPacketListener, FriendlyByteBuf> registerScreen(
            String id, BiConsumer<ClientPacketListener, FriendlyByteBuf> handler) {
    ...
    public static void handle(OpenScreenPacket packet) {
        BiConsumer<ClientPacketListener, FriendlyByteBuf> handler = SCREEN_IDS.get(packet.screenName);
        if (handler != null) {
            handler.accept(packet.packetListenerSupplier.get(), packet.buf);
        }
    }
