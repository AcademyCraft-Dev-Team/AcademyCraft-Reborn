...
import static org.academy.internal.client.gui.AbilityDeveloperScreen.RenderTypes.*;
import static org.academy.internal.client.gui.AbilityDeveloperScreen.Textures.*;
...
public class AbilityDeveloperScreen extends CGuiScreen {
    ...
    public static void initS2CPacket() {
        NetworkSystemClient.registerS2CPacketHandler(Packets.S2C_OPEN_ABILITY_DEVELOPER_SCREEN, (listener, packet) -> {
    ...
    protected void onInit() {
        {
            ...
            rootContainer.addChild("panel_main", mainPanel);
            {
                ...
                mainPanel.addChild("panel_right", rightPanel);
                {
                    ...
                    {
                        TypewriterLabelWidget welcome = new TypewriterLabelWidget("Welcome to Academy OS, Ver 0.0.1", PANEL_RIGHT_SKILL_BACK_X, PANEL_RIGHT_SKILL_BACK_Y);
                        bootPanel.addChild("welcome", welcome);
                        String userName = "None";
                        if (Minecraft.getInstance().player != null) {
                            userName = Minecraft.getInstance().player.getName().getString();
                        }
                        TypewriterLabelWidget userDetected = new TypewriterLabelWidget(String.format("User %s detected, ", userName), welcome.getX(), welcome.getY() + welcome.getHeight());
                        bootPanel.addChild("user_detected", userDetected);
                        TypewriterLabelWidget systemBooting = new TypewriterLabelWidget("System booting...", userDetected.getX(), userDetected.getY() + userDetected.getHeight());
                        bootPanel.addChild("system_booting", systemBooting);
                        TypewriterLabelWidget loadingText = new TypewriterLabelWidget("Loading: ", systemBooting.getX(), systemBooting.getY() + systemBooting.getHeight());
                        bootPanel.addChild("loading_text", loadingText);
                        BracketProgressBarWidget loadingBar = new BracketProgressBarWidget('#', 10, loadingText.getX(), loadingText.getY() + loadingText.getHeight());
                        loadingBar.updateInterval = 2;
                        bootPanel.addChild("loading_bar", loadingBar);
                        TypewriterLabelWidget invalid = new TypewriterLabelWidget("Invalid ability category, boot failed", loadingBar.getX(), loadingBar.getY() + loadingBar.getHeight());
                        ...
                        invalid.afterFinished = () -> {
                            TextBoxWidget textBox = new TextBoxWidget(32, invalid.getX(), invalid.getY() + invalid.getHeight(), PANEL_RIGHT_WIDTH - 16, invalid.getHeight());
        ...
        {
            PanelWidget wirelessPanel = new PanelWidget(0, 0, width, height);
            ...
            rootContainer.addChild(PANEL_WIRELESS_NAME, wirelessPanel);
            {
                ...
                PanelWidget viewListPanel = new PanelWidget(width / 2f - PANEL_WIRELESS_WIDTH / 2f, height / 2f - PANEL_MAIN_HEIGHT / 2f, PANEL_WIRELESS_WIDTH, PANEL_MAIN_HEIGHT);
                wirelessPanel.addChild("view", viewListPanel);
                {
                    ...
                    viewListPanel.addChild("connected_node_label", connectedLabel);
                    LabelWidget availableLabel = new LabelWidget("Available", 12, 54);
                    viewListPanel.addChild("available_node_label", availableLabel);
                    nodeListPanel = new SmoothScrollPanelWidget(10, 64, 160, 114);
                    viewListPanel.addChild("node_list", nodeListPanel);
                    VerticalScrollBarWidget scrollBar = new VerticalScrollBarWidget(nodeListPanel, 160, 64, 5, 114);
                    viewListPanel.addChild("scroll_bar", scrollBar);
    ...
    private void requestAvailableNodes(SmoothScrollPanelWidget listPanel) {
        FutureManagerClient.<List<String>>sendFuturePacket(Packets.C2S_GET_AVAILABLE_NODE_LIST, availableNodeNames -> {
            ...
            availableNodeNames.removeIf(s -> s.equals(currentlyConnectedNodeName));
    ...
    private void requestCurrentNodeStatus() {
        FutureManagerClient.<Pair<Boolean, String>>sendFuturePacket(Packets.C2S_GET_CURRENT_NODE, pair -> updateConnectedNodeDisplay(pair.getLeft(), pair.getRight()), mainPos);
    }

    private void updateConnectedNodeDisplay(boolean isNull, String nodeName) {
        boolean changed = !this.currentlyConnectedNodeName.equals(nodeName);
        this.currentlyConnectedNodeName = nodeName;
        AbstractContainerWidget viewPanel = rootContainer.<AbstractContainerWidget>getChildUnSafe(PANEL_WIRELESS_NAME).getChildUnSafe("view");
        viewPanel.removeChild("connected_node");
        PanelWidget connectedNodeWidgetRef = getNodeWidget(12, 38, nodeName, true, isNull);
        viewPanel.addChild("connected_node", connectedNodeWidgetRef);
        AbstractContainerWidget leftPanel = rootContainer.<AbstractContainerWidget>getChildUnSafe("panel_main").getChildUnSafe("panel_left");
        PanelButtonWidget wirelessButtonPanel = leftPanel.getChildUnSafe("button_wireless");
        LabelWidget nameLabel = (LabelWidget) wirelessButtonPanel.getChildren().get("label_name");
        nameLabel.value = nodeName;
        if (changed) {
            AbstractContainerWidget wirelessRoot = rootContainer.getChildUnSafe(PANEL_WIRELESS_NAME);
            if (wirelessRoot.isVisible()) {
                AbstractContainerWidget viewPanelForRefresh = wirelessRoot.getChildUnSafe("view");
                SmoothScrollPanelWidget nodeListPanel = viewPanelForRefresh.getChildUnSafe("node_list");
                requestAvailableNodes(nodeListPanel);
            }
        }
    }
        ...
        PanelWidget nodeViewPanel = new PanelWidget(x, y, 158, 16);
        {
            ...
            if (!isConnected) {
                ...
                ImageButtonWidget buttonWidget = new ImageButtonWidget(118, 1, 14, 14, RENDER_TYPE_ICON_UNCONNECTED, () -> {
                    ...
                    NetworkSystemClient.sendPacket(new C2SPacket(Packets.C2S_CONNECT_NODE, mainPos, nodeName, password));
                    requestCurrentNodeStatus();
                });
                ...
                if (!isNull) {
                    ...
                    {
                        NetworkSystemClient.sendPacket(new C2SPacket(Packets.C2S_DISCONNECT_NODE, mainPos));
                        requestCurrentNodeStatus();
                        requestAvailableNodes(nodeListPanel);
                    }
                    );
    ...
    private @NotNull PanelWidget getLeftPanel() {
        ...
        {
            ImageWidget leftPanelBackMiddle = new ImageWidget(0, 0, PANEL_LEFT_WIDTH, PANEL_MAIN_HEIGHT, RENDER_TYPE_PANEL_LEFT_BACK_MIDDLE);
            leftPanel.addChild("left_panel_back_middle", leftPanelBackMiddle);
            ImageWidget leftPanelBackBottom = new ImageWidget(0, 0, PANEL_LEFT_WIDTH, PANEL_MAIN_HEIGHT, RENDER_TYPE_PANEL_LEFT_BACK_BOTTOM);
            ...
            LabelWidget wirelessLabel = new LabelWidget("Current Node:", 8, 108);
            leftPanel.addChild("label_wireless", wirelessLabel);
            PanelButtonWidget wirelessButtonPanel = new PanelButtonWidget(8, 118, 90, 16, () -> {
                AbstractContainerWidget wirelessRoot = rootContainer.getChildUnSafe(PANEL_WIRELESS_NAME);
                wirelessRoot.setVisible(true);
                wirelessRoot.setEnabled(true);
                requestCurrentNodeStatus();
                AbstractContainerWidget viewListPanel = wirelessRoot.getChildUnSafe("view");
                SmoothScrollPanelWidget nodeListPanel = viewListPanel.getChildUnSafe("node_list");
                requestAvailableNodes(nodeListPanel);
            });
    ...
    static final class Textures {
        static final ResourceLocation PANEL_LEFT_BACK_TOP_TEXTURE = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/ui_developerleft.png");
        static final ResourceLocation TEXTURE_PANEL_LEFT_BACK_MIDDLE = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/parent_background_developermachine.png");
        static final ResourceLocation TEXTURE_PANEL_LEFT_BACK_BOTTOM = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/parent_background_developerleft.png");
        static final ResourceLocation TEXTURE_PANEL_RIGHT_BACK = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/parent_background_developerright.png");
        static final ResourceLocation TEXTURE_PANEL_RIGHT_INFO = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/ui_developerright.png");
        static final ResourceLocation TEXTURE_PANEL_RIGHT_SKILL_BACK = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/skill_panel_back.png");
        static final ResourceLocation TEXTURE_PANEL_RIGHT_SKILL_ICON_BACK = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/developer/skill_back.png");
        static final ResourceLocation TEXTURE_ELEMENT_LINE = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/element/line.png");
        static final ResourceLocation TEXTURE_ELEMENT_BACK_DARK = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/element/element_background_dark.png");
        static final ResourceLocation TEXTURE_ELEMENT_BACK_LIGHT = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/element/element_background_light.png");
        static final ResourceLocation TEXTURE_WIRELESS_PANEL_VIEW_ICON = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/icon/icon_tonode.png");
        static final ResourceLocation TEXTURE_ICON_NODE = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/icon/icon_node.png");
        static final ResourceLocation TEXTURE_ICON_CONNECTED = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/icon/icon_connected.png");
        static final ResourceLocation TEXTURE_ICON_UNCONNECTED = new ResourceLocation(AcademyCraft.MOD_ID, "textures/gui/icon/icon_unconnected.png");
    }
    ...
    static final class RenderTypes {
        ...
        static final RenderType RENDER_TYPE_ELEMENT_BACK_LIGHT = RenderUtil.getPositionTexRenderType(
        ...
        static final RenderType RENDER_TYPE_ICON_NODE = RenderUtil.getPositionTexRenderType(
                "icon_node", TEXTURE_ICON_NODE, false);
        static final RenderType RENDER_TYPE_ICON_CONNECTED = RenderUtil.getPositionColorTexRenderType(
                "icon_connected", TEXTURE_ICON_CONNECTED, false);
        static final RenderType RENDER_TYPE_ICON_UNCONNECTED = RenderUtil.getPositionColorTexRenderType(
