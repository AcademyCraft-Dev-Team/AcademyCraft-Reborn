...
public class PlasmaRenderer extends EntityRenderer<Plasma> {
    ...
    private static final ResourceLocation PLASMA_PARTICLE_TEXTURE = new ResourceLocation(AcademyCraft.MOD_ID, "textures/skill/effect/generic/sparkle_blurred.png"); // 等离子体粒子纹理
    private static final ResourceLocation ATMOSPHERE_TEXTURE = new ResourceLocation(AcademyCraft.MOD_ID, "textures/skill/effect/generic/white_smoke_hq.png"); // 大气效果纹理
    ...
    private final float[][][] cachedLayerVertexBuffer;
    ...
    public PlasmaRenderer(EntityRendererProvider.Context context) {
        ...
        this.cachedLayerVertexBuffer = RenderUtil.RingRenderer.getVerticalVertexBuffer(1.0f, 1.0f, LAYER_SEGMENTS);
    }
    ...
    private void renderAtmosphericLayers(PoseStack poseStack, MultiBufferSource buffer, float effectiveTime, long entityId, float gatherFactor) {
        ...
        for (int i = 0; i < NUM_LAYERS; ++i) {
            ...
            RenderUtil.RingRenderer.renderVerticalRing(poseStack.last().pose(), atmosphereConsumer, LAYER_SEGMENTS, this.cachedLayerVertexBuffer);
    ...
    private float calculateGap(int layerIndex, double timeParamDyn, float averageGap) {
        ...
        return Math.max(averageGap * 0.1f, variedGap); // Ensure minimum gap slightly smaller
    }
    ...
    private double calculateVariedRotation(double normalizedYRelCore, int layerIndex, double timeParamRot, float gatherFactor) {
        ...
        // Fixed direction (e.g., always positive)
        double direction = 1.0;

        double baseRotationSpeed = Math.toRadians(1.0) * baseSpeedMultiplier * direction;
    ...
    private float calculateLayerWidth(double normalizedYRelCore, int layerIndex, double timeParamDyn) {
        ...
        return Math.max(3.0f, finalWidth); // Slightly reduce minimum width
    }
